...
class DbModel extends \ObjectModel
{
    ...
    private static function _makeColSqlFromDefination()
    {
        ...
        foreach (static::$definition['fields'] as $col_name => $col_info) {
            $col_sql .= "`{$col_name}` " .
                ...
                (isset($col_info['auto_date']) ? ($col_info['auto_date'] == 'insert' ? ' DEFAULT CURRENT_TIMESTAMP '
                    :
    ...
    private function _reformatFields()
    {
        ...
        array_walk($fields, function ($field_value, $field_name) use (&$fields, &$autodate_fields) {
    ...
    public function insert($null_values = true, $force_id = false, $insert_type = DB::INSERT)
    {
        ...
        if (!$result = Db::getInstance()->insert($this->def['table'], $fields, $null_values, FALSE, $insert_type, static::$_PREFIX)) {
            return false;
        }
    ...
    public function update($null_values = true)
    {
        ...
        if (!$result = Db::getInstance()->update($this->def['table'], $fields, '`' . pSQL($this->def['primary']) . '` = ' . (int)$this->id, 0, $null_values, FALSE, Db::INSERT, static::$_PREFIX)) {
            return false;
        }
    ...
    {
        if (!$result = Db::getInstance()->delete($this->def['table'], '`' . pSQL($this->def['primary']) . '` = ' . (int)$this->id, 0, FALSE, static::$_PREFIX)) {
            return false;
        }
    ...
    {
        $id = Db::getInstance()->getRow('SELECT * FROM ' . static::_getTableName() .
            ($where != NULL ? " WHERE {$where}" : '') .
            (
            $order_by != null ? ' ORDER BY ' .
    ...
    public static function selectAll(string $where = NULL, array $select_columns = NULL)
    {
        return (Db::getInstance()->executeS('SELECT * FROM ' . static::_getTableName() . ($where != NULL ? " WHERE {$where}" : '')));
    }
    ...
    {
        return (Db::getInstance()->getRow('SELECT * FROM ' . static::_getTableName() .
            ($where != NULL ? " WHERE {$where}" : '') .
    ...
    {
        return (Db::getInstance()->getValue('SELECT ' . $select_column . ' FROM ' .
            static::_getTableName() .
            " WHERE {$where} " .
            ($order_by != null ? ' ORDER BY ' . implode(',', array_map(function ($column) {
                    return implode(' ', $column);
                }, $order_by)) : '')
        ));
    }
    ...
    {
        return (Db::getInstance()->getValue('SELECT COUNT(*) FROM ' . static::_getTableName() . " WHERE {$where}"));
    }

    public static function deleteRow(string $where)
    {
        return (Db::getInstance()->delete(static::$definition['table'], $where, 0, FALSE, static::$_PREFIX));
    }

    public static function insertRow(array $fields, $insert_type = Db::INSERT)
    {
        return (Db::getInstance()->insert(static::$definition['table'], $fields, TRUE, FALSE, $insert_type, static::$_PREFIX));
    }

    public static function updateRow(array $fields, string $where)
    {
        return (Db::getInstance()->update(static::$definition['table'], $fields, $where, 0, TRUE, FALSE, static::$_PREFIX));
    }
